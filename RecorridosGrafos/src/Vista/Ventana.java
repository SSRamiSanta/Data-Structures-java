...
public class Ventana extends JFrame implements ActionListener, MouseListener, MouseMotionListener {
    ...
    private void Paneles() {
        ...
        P_Arbol.setBackground(Color.darkGray);
    ...
    private void Botones() {
        B_Ancho = new JButton("<html><center> Recorrido Ancho <html>");
        B_Ancho.setFont(new Font("Times new roman", Font.BOLD, 15));
        ...
        B_Profundidad.setFont(new Font("Times new roman", Font.BOLD, 15));
        ...
        B_Cerrar.setBounds(1280, 10, 40, 30);
    ...
    public boolean guardarMatriz(int f, int c) {
        for (int i = 0; i < f; i++) {
            for (int j = 0; j < c; j++) {
                ...
                if (matrizAd[i][j] != 0) {
                    if (matrizAd[i][j] == (i + 1) || !gl.getVertices().contains(matrizAd[i][j])) {
                        return false;
                    }
                }
